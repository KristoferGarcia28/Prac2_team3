    ...
    uint32_t dataLen)
{
    ...
    shell_printf("coap %s %s %s %s\r\n",
                pSession->msgType == gCoapConfirmable_c ? "CON" : "NON",
                pSession->code == gCoapGET_c ? "GET" :
                pSession->code == gCoapPOST_c ? "POST" :
                pSession->code == gCoapPUT_c ? "PUT" : "UNKNOWN",
                addrStr,
    ...
    if (pSession->msgType == gCoapConfirmable_c) {
        ...
        // Imprimir el valor del contador que estamos enviando
        shell_printf("Responding with counter value: %d\r\n", team3Counter);
