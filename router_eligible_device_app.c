...
#define APP_STOP_TIMER_URI_PATH               			"/stopTmr"
#define APP_RESTART_TIMER_URI_PATH            			"/restartTmr"
...
static bool_t mTeam3RequestsEnabled = FALSE;
...
bool LeerAcelerometro(int16_t *x, int16_t *y, int16_t *z);
...
//parte 2
const coapUriPath_t gAPP_STOP_TIMER_URI_PATH = {SizeOfString(APP_STOP_TIMER_URI_PATH), (uint8_t *)APP_STOP_TIMER_URI_PATH};
const coapUriPath_t gAPP_RESTART_TIMER_URI_PATH = {SizeOfString(APP_RESTART_TIMER_URI_PATH), (uint8_t *)APP_RESTART_TIMER_URI_PATH};
...
static uint32_t gCurrentTimerIntervalMs = 1000; // Valor por defecto del timer: 1 segundo
...
static void Team3_TimerCallback(void *param)
{
    ...
    {
        TMR_StartSingleShotTimer(team3TimerId, gCurrentTimerIntervalMs, Team3_TimerCallback, NULL);
    }
}
    ...
    uint32_t dataLen)
{
    ...
    // 1. Activar peticiones periódicas si no están activas
    if (!mTeam3RequestTimerRunning) {
        mTeam3RequestsEnabled = TRUE;
        APP_StartTeam3Requests();
        shell_printf("Team3 requests STARTED from %s\r\n", addrStr);
    }

    // 2. Si es GET, responder con el contador actual
    if (pSession->code == gCoapGET_c) {
        counterLen = snprintf(counterStr, sizeof(counterStr), "%d", team3Counter);
        shell_printf("Responding to %s with counter: %d\r\n", addrStr, team3Counter);
        COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, (uint8_t*)counterStr, counterLen);
    }
    // 3. Si es POST (u otro), solo confirmar
    else if (pSession->msgType == gCoapConfirmable_c) {
    ...
    uint32_t dataLen)
{
    if(sessionStatus == gCoapSuccess_c && pData && dataLen > 0)
    {
        ...
        shell_printf("Received response - Counter = %.*s from %s type %s\r\n",
                     dataLen, pData,
                     addrStr,
    ...
    {
        shell_printf("Failed to get counter value from leader\r\n");
    }
}

static void APP_CoapTeam3ControlCb(
    coapSessionStatus_t sessionStatus,
    uint8_t *pData,
    coapSession_t *pSession,
    uint32_t dataLen)
{
    ...
    if (sessionStatus == gCoapSuccess_c)
    {
        ...
        shell_printf("Team3 requests ENABLED from %s type %s\r\n",
                    addrStr,
                    pSession->msgType == gCoapConfirmable_c ? "CON" : "NON");
    }
...
void APP_StartTeam3Requests(void)
{
    ...
    if(mTeam3RequestTimerId != gTmrInvalidTimerID_c && mTeam3RequestsEnabled)
    {
...
static void APP_CoapStopTimerCb(
    coapSessionStatus_t sessionStatus,
    uint8_t *pData,
    coapSession_t *pSession,
    uint32_t dataLen)
{
    ...
    {
        Team3_StopTimer();
        shell_printf("Timer stopped from %s type %s Count= %d\r\n",
                    addrStr,
                    pSession->msgType == gCoapConfirmable_c ? "CON" : "NON",
                    team3Counter);
    }
...
static void APP_CoapAccelCb(
    ...
    uint32_t dataLen)
{
    if (sessionStatus == gCoapSuccess_c && pSession->code == gCoapGET_c)
    {
        char addrStr[INET6_ADDRSTRLEN];
        ntop(AF_INET6, (ipAddr_t*)&pSession->remoteAddrStorage.ss_addr, addrStr, INET6_ADDRSTRLEN);
        ...
        // Leer acelerómetro
        int16_t x, y, z;
        if (LeerAcelerometro(&x, &y, &z))
        {
            char response[64];
            snprintf(response, sizeof(response), "X=%d Y=%d Z=%d", x, y, z);

            // Imprimir en shell del líder
            shell_printf("Accel data sent to %s (%s request)\r\n",
                        addrStr,
                        pSession->msgType == gCoapConfirmable_c ? "CON" : "NON");
...
static void APP_AccelResponseHandler(
    coapSessionStatus_t sessionStatus,
    uint8_t *pData,
    coapSession_t *pSession,
    uint32_t dataLen)
{
    if (sessionStatus == gCoapSuccess_c && pData) {
        char addrStr[INET6_ADDRSTRLEN];
        ntop(AF_INET6, (ipAddr_t*)&pSession->remoteAddrStorage.ss_addr, addrStr, INET6_ADDRSTRLEN);

        // Parsear datos (ej: "X=100 Y=100 Z=0")
        int16_t x, y, z;
        sscanf((char*)pData, "X=%hd Y=%hd Z=%hd", &x, &y, &z);

        // Imprimir en shell
        shell_printf("Received accel data: %s from %s (%s request)\r\n",
                    (char*)pData,
                    addrStr,
                    pSession->msgType == gCoapConfirmable_c ? "CON" : "NON");

        // Controlar LEDs según el eje dominante
        if (abs(z) >= abs(x) && abs(z) >= abs(y)) {
            LED_RedOn();  // Z es mayor
        }
        else if (abs(x) >= abs(y)) {
            LED_BlueOn(); // X es mayor
        }
        else {
            LED_GreenOn(); // Y es mayor
        }
    }
}
static void APP_SendAccelRequest(uint8_t *param)
{
    coapSession_t *pSession = COAP_OpenSession(mAppCoapInstId);
    if (pSession)
    {
        ...
        pSession->pCallback = APP_AccelResponseHandler;
        FLib_MemCpy(&pSession->remoteAddrStorage.ss_addr, &leaderAddr, sizeof(ipAddr_t));
        pSession->pUriPath = (coapUriPath_t *)&gAPP_ACCEL_URI_PATH;

        // Alternar entre CON/NON
        static bool_t useCon = TRUE;
        pSession->msgType = useCon ? gCoapConfirmable_c : gCoapNonConfirmable_c;
        useCon = !useCon;
...
static void APP_CoapRestartTimerCb(
    coapSessionStatus_t sessionStatus,
    uint8_t *pData,
    coapSession_t *pSession,
    uint32_t dataLen)
{
    ...
    {
        Team3_StartTimer();
        shell_printf("Timer started from %s type %s starting Count= %d\r\n",
                    addrStr,
                    pSession->msgType == gCoapConfirmable_c ? "CON" : "NON",
                    team3Counter);
    }
...
/*Parte extra*/
static void Team3_ChangeTimerInterval(uint32_t newIntervalSec)
{
    Team3_StopTimer(); // Detener el temporizador actual

    // Convertir segundos a milisegundos (que es lo que usa el timer)
    uint32_t intervalMs = newIntervalSec * 1000; // Cambiado de newIntervalMs a intervalMs

    if (team3TimerId != gTmrInvalidTimerID_c)
    {
        team3TimerRunning = TRUE;
        TMR_StartSingleShotTimer(team3TimerId, intervalMs, Team3_TimerCallback, NULL);

        // Actualizar el intervalo global
        gCurrentTimerIntervalMs = intervalMs;
    }
}

static void APP_CoapChangeTimeCb(
    coapSessionStatus_t sessionStatus,
    uint8_t *pData,
    coapSession_t *pSession,
    uint32_t dataLen)
{
    ...
    if (sessionStatus == gCoapSuccess_c && pData && dataLen > 0)
    {
        uint32_t newInterval = atoi((char *)pData);

        if (newInterval > 0 && newInterval <= 60) // Validar rango razonable
        {
            Team3_ChangeTimerInterval(newInterval);

            shell_printf("Timer interval changed to %d seconds from %s type %s\r\n",
                        newInterval,
                        addrStr,
                        pSession->msgType == gCoapConfirmable_c ? "CON" : "NON");
        }
        ...
        {
            shell_printf("Invalid interval value: %d (must be 1-60)\r\n", newInterval);
        }
    }
...
i2c_master_handle_t g_MasterHandle;
static uint8_t dataScale = 0; // Escala de los datos del acelerómetro

/*******************************************************************************
 * Función para leer el acelerómetro
 ******************************************************************************/
...
bool LeerAcelerometro(int16_t *x, int16_t *y, int16_t *z) {
    ...
    // Procesar datos X, Y, Z (14 bits con signo)
    *x = (int16_t)(((uint16_t)sensorData.accelXMSB << 8) | (uint16_t)sensorData.accelXLSB) / 4U;
    *y = (int16_t)(((uint16_t)sensorData.accelYMSB << 8) | (uint16_t)sensorData.accelYLSB) / 4U;
    *z = (int16_t)(((uint16_t)sensorData.accelZMSB << 8) | (uint16_t)sensorData.accelZLSB) / 4U;

    return true; // Éxito
}

/*******************************************************************************
 * Inicialización del acelerómetro (se llama una vez al inicio)
 ******************************************************************************/
bool InicializarAcelerometro() {
    const uint8_t g_accel_address[] = {0x1CU, 0x1DU, 0x1EU, 0x1FU};
    uint8_t sensorRange = 0, regResult = 0;
    bool foundDevice = false;
    i2c_master_config_t i2cConfig;
    uint32_t i2cSourceClock = CLOCK_GetFreq(ACCEL_I2C_CLK_SRC);

    // Configuración inicial de I2C
    ...
    // Buscar dispositivo
    for (uint8_t i = 0; i < sizeof(g_accel_address); i++) {
    ...
    if (!foundDevice) {
        PRINTF("Error: Acelerómetro no encontrado.\r\n");
        return false;
    }

    // Inicializar sensor y configurar escala
    if (FXOS_Init(&fxosHandle) != kStatus_Success) {
        PRINTF("Error: Fallo al inicializar el acelerómetro.\r\n");
        return false;
    }
    ...
    // Determinar escala (2g, 4g, 8g)
    dataScale = (sensorRange == 0x00) ? 2 : (sensorRange == 0x01) ? 4 : 8;
    return true;
}
...
)
{
    ...
    if(gThrStatus_Success_c == THR_StartInstance(mThrInstanceId, pStackCfg[0]))
    {
        ...
        APP_InitCoapDemo();
        if (!InicializarAcelerometro())
        {
            shell_printf("Error: Acelerómetro no inicializado\r\n");
        }
...
)
{
    coapRegCbParams_t cbParams[] =  {{APP_CoapLedCb,  (coapUriPath_t *)&gAPP_LED_URI_PATH},
									 ...
									 {APP_CoapStopTimerCb, (coapUriPath_t *)&gAPP_STOP_TIMER_URI_PATH},
									 {APP_CoapRestartTimerCb, (coapUriPath_t *)&gAPP_RESTART_TIMER_URI_PATH},
